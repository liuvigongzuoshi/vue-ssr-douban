<template>
  <div class="book-type-content">
    ...
    <ul class="book-list">
      <li v-for="book in currentTypeBooks" :key="book.id" class="book-item">
        <a :href="book.alt" :title="book.title">
          <img :src="book.images.large" :alt="book.title" class="book-image" referrerpolicy="no-referrer" />
        </a>
...
export default {
  ...
  methods: {
    changeBookType(bookType) {
      this.$store.commit('book/SET_CURRENT_BOOK_TYPE', bookType)
      if (!this.typeBooks[bookType]) {
        this.$store.dispatch('book/getTypeBooks', { count: 12 })
      } else {
        this.$store.commit('book/SET_CURRENT_TYPE_BOOKS', this.typeBooks[bookType])
      }
    },
