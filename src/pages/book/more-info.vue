      ...
      <li v-for="book in currentTagBooks" :key="book.id" class="book-item">
        ...
        <div class="book-info">
          ...
          <p class="book-other-info">
            {{ book.author.join() }} / {{ book.translator.join() }} / {{ book.publisher }} / {{ book.pubdate }} /
            {{ book.price }}
          </p>
          <div>
...
export default {
  ...
  methods: {
    ...
    loadMoreBooks() {
      ...
      const bookCount = this.$store.state.book.currentTagBooks.length
      this.bookCountPerPage += 10
      if (bookCount < this.bookCountPerPage) {
        this.$store.dispatch('book/getCurrentTagBooks', {
