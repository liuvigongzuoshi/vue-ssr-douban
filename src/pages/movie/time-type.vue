...
export default {
  ...
  methods: {
    ...
    changeMovieTimeType(type) {
      this.$store.commit('movie/SET_CURRENT_MOVIE_TIME_TYPE', type)
      if (!this.timeTypeMovies[type.value]) {
        this.$store.dispatch('movie/getTimeTypeMovies', { start: 0, count: 48, searchParams: type.value })
      } else {
        this.$store.commit('movie/SET_CURRENT_TIME_TYPE_MOVIES', this.timeTypeMovies[type.value])
      }
    },
