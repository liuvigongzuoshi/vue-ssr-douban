...
export default {
  ...
  methods: {
    changeCurrentBookTags(tags) {
      this.$store.commit('book/SET_CURRENT_BOOK_TAGS', tags)
      this.$store.commit('book/SET_CURRENT_BOOK_TAG', tags.subTags[0])
      if (!this.tagBooks[tags.subTags[0]]) {
        this.$store.dispatch('book/getCurrentTagBooks', { count: 40 })
      } else {
        this.$store.commit('book/SET_CURRENT_TAG_BOOKS', { books: this.tagBooks[tags.subTags[0]] })
      }
    },

    changeCurrentBookTag(subTag) {
      this.$store.commit('book/SET_CURRENT_BOOK_TAG', subTag)
      if (!this.tagBooks[subTag]) {
        this.$store.dispatch('book/getCurrentTagBooks', { count: 40 })
      } else {
        this.$store.commit('book/SET_CURRENT_TAG_BOOKS', { books: this.tagBooks[subTag] })
      }
    }
  }
}
